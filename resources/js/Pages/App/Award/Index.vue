<template>
    ...
    <div class="flex flex-row flex-wrap items-stretch mb-8">
        <Card title="Membership" :href="route('award.membership.show')" progressBarId="membership-progress"
        ...
        <Card title="ICV List" :href="route('award.icv-list.show')" progressBarId="icv-progress"
            ...
            You must:<br>
            Complete six activities in total, two from each topic area of the ICV activities list. A minimum of two must
            be from the QSA ICV list and the others from any of the QSA, or
        ...
        <Card title="Generate Reports" :href="route('award.reports.show')" class="xl:pl-2">
            <span
                v-if="qsa_complete">Congratulations on completing your award. Click here to generate a completion report of your
                progress to share with others, including Scout Headquarters to apply for your award.</span>
            <span v-else>You can download a progress report on this page with the current details of your award progress. Then, when you've completed your award you come back here to generate a completion report with all the details of your award.</span>
        </Card>
    </div>
</template>
...
import Card from "@/Components/Award/Card";

export default defineComponent({
    components: {Head, Card},
    props: {
        qsa_complete: {
            type: Boolean,
            required: false,
